...
public class PasswordProcessorSerial {
    public static void main(String[] args) {
        if (args.length < 1) {
            System.out.println("Uso: java PasswordProcessorVirtualThreads <caminho_do_diretorio>");
            return;
        }
        ...
        for (File file : files) {
            // Cria e inicia uma thread virtual para cada arquivo
            Thread thread = Thread.ofVirtual().start(() -> processFile(file));
            threads.add(thread);
        }
